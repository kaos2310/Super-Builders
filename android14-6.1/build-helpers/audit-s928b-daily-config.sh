...
REQUIRE_SUSFS="${5:-true}"
REQUIRE_ZEROMOUNT="${6:-true}"
case "$KMI_MODE" in
...
if [[ "$REQUIRE_ZEROMOUNT" == "true" ]]; then
  REQUIRED+=(CONFIG_ZEROMOUNT)
fi
...
echo "SUSFS audit requirement: $REQUIRE_SUSFS"
echo "ZeroMount audit requirement: $REQUIRE_ZEROMOUNT"
if [[ "$KMI_MODE" == "strict" ]]; then
