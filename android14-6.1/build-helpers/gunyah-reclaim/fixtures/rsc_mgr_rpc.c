...
{
	size_t msg_size = 0, initial_mem_entries = p->n_mem_entries, resp_size = 0;
	...
	*attr_section = 0;
	...
	pr_info("GH_DIAG rm_mem_share call begin rpc=%#x label=%u total=%zu initial=%zu append=%u msg_size=%zu\n",
	    (unsigned int)message_id, p->label, p->n_mem_entries, initial_mem_entries,
	...
	pr_info("GH_DIAG rm_mem_share call end rpc=%#x label=%u ret=%d resp_size=%zu\n",
	    (unsigned int)message_id, p->label, ret, resp_size);
	kfree(msg);
	...
	pr_info("GH_DIAG rm_mem_share handle rpc=%#x label=%u handle=%u\n",
	    (unsigned int)message_id, p->label, p->mem_handle);
	kfree(resp);
