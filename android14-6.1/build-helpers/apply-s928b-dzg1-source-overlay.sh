...
OVERLAY_ARCHIVE="${2:?Samsung DZG1 source overlay archive}"
...
[[ -f "$OVERLAY_ARCHIVE" ]] || {
  echo "::error::Samsung source overlay not found: $OVERLAY_ARCHIVE"
  exit 1
}

ACTUAL_AOSP_COMMIT=$(git -C "$COMMON_TREE" rev-parse HEAD)
[[ "$ACTUAL_AOSP_COMMIT" == "$EXPECTED_AOSP_COMMIT" ]] || {
  echo "::error::Samsung overlay base mismatch: $ACTUAL_AOSP_COMMIT != $EXPECTED_AOSP_COMMIT"
  exit 1
}
[[ -z "$(git -C "$COMMON_TREE" status --porcelain --untracked-files=all)" ]] || {
  echo "::error::Samsung source overlay requires a clean, freshly synced AOSP common tree"
...
[[ "$ACTUAL_OVERLAY_SHA256" == "$EXPECTED_OVERLAY_SHA256" ]] || {
  echo "::error::Samsung source overlay SHA256 mismatch"
...
expected_fields = ["status", "path", "aosp_git_blob", "official_git_blob", "size"]
if reader.fieldnames != expected_fields:
    raise SystemExit(f"unexpected Samsung overlay manifest fields: {reader.fieldnames}")

manifest: dict[str, dict[str, str]] = {}
status_counts = {"changed": 0, "official-only": 0}
for row in reader:
    ...
    if len(row["official_git_blob"]) != 40:
        raise SystemExit(f"invalid official Git blob for {relative}")
    ...
    if git_blob(data) != row["official_git_blob"]:
        raise SystemExit(f"Samsung overlay Git blob mismatch: {relative}")
...
if status_counts != {"changed": 405, "official-only": 421}:
...
if len(delete_paths) != 309 or len(set(delete_paths)) != 309:
...
for relative, row in manifest.items():
    target = root / safe_relative(relative)
    if git_blob(target.read_bytes()) != row["official_git_blob"]:
        raise SystemExit(f"post-apply Samsung source mismatch: {relative}")

print(
    "Applied exact Samsung common source delta: "
    f"{status_counts['changed']} changed, "
    f"{status_counts['official-only']} added, {len(delete_paths)} removed"
)
PY

for SPEC in \
  "include/trace/hooks/xhci.h:fda469c283497303230ff7a145d26cc16a80cd8ea5bf4b4eff23705b59da4f29" \
  "drivers/android/vendor_hooks.c:b82dcc365b85aca4d8413b3eab149fb1cd05170a5d67b48719642e54cff28fe2" \
  "drivers/usb/host/xhci-plat.c:f2c2fba8c4a04fdab9fbd840e38876e29a6a9fcfc2a1d8c1578cb822f1f873ac" \
  ...
  [[ "$ACTUAL" == "$EXPECTED" ]] || {
    echo "::error::Official Samsung source hash mismatch after overlay: $RELATIVE"
...
echo "Verified SM-S928B Android 16 source overlay against S928BXXS6DZG1"
